...
const props = defineProps({
  ...
  model: {
    request: true,
    type: Object as PropType<Debt.ReqUpdateLogParams>,
  },
})
...
const FormData = reactive({
  model: lodash.cloneDeep(props.model) as Debt.ReqUpdateLogParams,
  handleSubmit: () => {
    formRef.value.validate(async (errors: any) => {
      try {
        if (!errors) {
          await debtStore.apiDebtLogUpdate(FormData.model)
