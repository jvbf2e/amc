...
const props = defineProps({
  model: {
    type: Object as PropType<Debt.ReqUpdateServiceParams>,
  },
})
...
const FormData = reactive({
  model: lodash.cloneDeep(props.model as Debt.ReqUpdateServiceParams),
  handleSubmit: () => {
    formRef.value.validate(async (errors: any) => {
      try {
        if (!errors) {
          await debtStore.apiDebtServiceUpdate(FormData.model)
