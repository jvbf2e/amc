...
import { getDebtExecutorInfo, getDebtExecutorList } from '@/api/modules'
...
const Apis = reactive({
  getInfo: async (id: number) => {
    try {
      const { code, data: res, msg } = await getDebtExecutorInfo(id)
      if (code === 0) {
        WebData.info = res
      } else {
        throw msg
  ...
  getList: async () => {
    try {
      const {
        ...
        data: res,
        msg,
      } = await getDebtExecutorList(props.assetDebtId)
      if (code === 0) {
        TableData.data = res
      } else {
        throw msg
...
const WebData = reactive({
  info: {} as Debt.ResReadExecutorData,
})

const TableData = reactive({
  ...Config.table,
  data: [] as Debt.ResReadExecutorData[],
  handleDelete: async (record: any) => {
    try {
      await debtStore.apiDebtExecutorDelete(record.id)
      await Apis.getList()
    } catch (error) {
      appStore.setMessage({ content: error as string, type: 'danger' })
    }
...
const DrawerData = reactive({
  ...
  handleVisible: async (type: CrudType, record?: Debt.ResReadExecutorData) => {
    switch (type) {
      case 'create':
