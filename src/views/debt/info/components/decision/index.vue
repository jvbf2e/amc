...
import { getDebtDecisionInfo, getDebtDecisionList } from '@/api/modules'
...
const Apis = reactive({
  getInfo: async (id: number) => {
    try {
      const { code, data: res, msg } = await getDebtDecisionInfo(id)
      if (code === 0) {
        WebData.info = res
      } else {
        throw msg
  ...
  getList: async () => {
    try {
      const {
        ...
        data: res,
        msg,
      } = await getDebtDecisionList(props.assetDebtId)
      if (code === 0) {
        TableData.data = res
      } else {
        throw msg
...
const WebData = reactive({
  info: {} as Debt.ResReadDecisionData,
})

const TableData = reactive({
  ...Config.table,
  data: [] as Debt.ResReadDecisionData[],
  handleDelete: async (record: any) => {
    try {
      await debtStore.apiDebtDecisionDelete(record.id)
      await Apis.getList()
    } catch (error) {
      appStore.setMessage({ content: error as string, type: 'danger' })
    }
...
const DrawerData = reactive({
  ...
  handleVisible: async (type: CrudType, record?: Debt.ResReadDecisionData) => {
    switch (type) {
      case 'create':
