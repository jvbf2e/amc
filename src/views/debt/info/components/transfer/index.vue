...
import { getDebtTransferInfo, getDebtTransferList } from '@/api/modules'
...
const Apis = reactive({
  getInfo: async (id: number) => {
    try {
      const { code, data: res, msg } = await getDebtTransferInfo(id)
      if (code === 0) {
        WebData.info = res
      } else {
        throw msg
  ...
  getList: async () => {
    try {
      const {
        ...
        data: res,
        msg,
      } = await getDebtTransferList(props.assetDebtId)
      if (code === 0) {
        TableData.data = res
      } else {
        throw msg
...
const WebData = reactive({
  info: {} as Debt.ResReadTransferData,
})

const TableData = reactive({
  ...Config.table,
  data: [] as Debt.ResReadTransferData[],
  handleDelete: async (record: any) => {
    try {
      await debtStore.apiDebtTransferDelete(record.id)
      await Apis.getList()
    } catch (error) {
      appStore.setMessage({ content: error as string, type: 'danger' })
    }
...
const DrawerData = reactive({
  ...
  handleVisible: async (type: CrudType, record?: Debt.ResReadTransferData) => {
    switch (type) {
      case 'create':
