...
const props = defineProps({
  model: {
    type: Object as PropType<Debt.ReqUpdateTransferParams>,
  },
})
...
const FormData = reactive({
  model: lodash.cloneDeep(props.model as Debt.ReqUpdateTransferParams),
  handleSubmit: () => {
    formRef.value.validate(async (errors: any) => {
      try {
        if (!errors) {
          await debtStore.apiDebtTransferUpdate(FormData.model)
