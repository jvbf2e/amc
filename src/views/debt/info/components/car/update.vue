...
const props = defineProps({
  model: {
    type: Object as PropType<Debt.ReqUpdateCarParams>,
  },
})
...
const FormData = reactive({
  model: lodash.cloneDeep(props.model as Debt.ReqUpdateCarParams),
  handleSubmit: () => {
    formRef.value.validate(async (errors: any) => {
      try {
        if (!errors) {
          await debtStore.apiDebtCarUpdate(FormData.model)
