...
const props = defineProps({
  model: {
    type: Object as PropType<Debt.ReqUpdateHouseParams>,
  },
})
...
const FormData = reactive({
  model: lodash.cloneDeep(props.model as Debt.ReqUpdateHouseParams),
  loading: false,
  ...
  handleSubmit: () => {
    formRef.value.validate(async (errors: any) => {
      try {
        if (!errors) {
          FormData.model.file = WebData.files.map((item: any) => item.path)
          await debtStore.apiDebtHouseUpdate(FormData.model)
...
onMounted(async () => {
  WebData.files = props.model?.file.map((item: any) => {
    return {
      file_name: item.name,
      file_path: item.file,
      url: `${IMAGE_URL}/${item.file}`,
    }
  }) as { file_name: string; file_path: string; url: string }[]
  FormData.model.housing_types === 0 && (FormData.model.housing_types = '')
  FormData.model.produce_status === 0 && (FormData.model.produce_status = '')
})
