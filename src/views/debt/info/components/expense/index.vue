...
import { getDebtExpenseInfo, getDebtExpenseList } from '@/api/modules'
...
const Apis = reactive({
  getInfo: async (id: number) => {
    try {
      const { code, data: res, msg } = await getDebtExpenseInfo(id)
      if (code === 0) {
        WebData.info = res
      } else {
        throw msg
  ...
  getList: async () => {
    try {
      const {
        ...
        data: res,
        msg,
      } = await getDebtExpenseList(props.assetDebtId)
      if (code === 0) {
        TableData.data = res
      } else {
        throw msg
...
const WebData = reactive({
  info: {} as Debt.ResReadExpenseData,
})

const TableData = reactive({
  ...Config.table,
  data: [] as Debt.ResReadExpenseData[],
  handleDelete: async (record: any) => {
    try {
      await debtStore.apiDebtExpenseDelete(record.id)
      await Apis.getList()
    } catch (error) {
      appStore.setMessage({ content: error as string, type: 'danger' })
    }
...
const DrawerData = reactive({
  ...
  handleVisible: async (type: CrudType, record?: Debt.ResReadExpenseData) => {
    switch (type) {
      case 'create':
