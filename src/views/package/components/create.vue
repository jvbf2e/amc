...
const WebData = reactive({
  handleChange: (value: any) => {
    FormData.model.profit_target =
      (FormData.model.collection_target ?? 0) - value
  },
  handleChange2: (value: any) => {
    FormData.model.profit_target = value - (FormData.model.invest_amount ?? 0)
  },
  change: (value: any) => {
    console.log(value)
  },
...
const FormData = reactive({
  ...
  handleSubmit: () => {
    formRef.value.validate(async (errors: any) => {
      try {
        if (!errors) {
          await packageStore.apiPackageCreate(FormData.model)
