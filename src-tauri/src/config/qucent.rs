...
pub struct IQucentTemp(pub Mapping);

impl IQucentTemp {
  pub fn new() -> Self {
    match dirs::qucent_path().and_then(|path| help::read_merge_mapping(&path)) {
  ...
  pub fn save_config(&self) -> Result<()> {
    help::save_yaml(
      &dirs::qucent_path()?,
      &self.0,
      Some("# Generated by Qucent Verge"),
    )
  }
  ...
  pub fn get_client_info(&self) -> QucentInfo {
    let config = &self.0;
...
#[test]
fn test_qucent_info() {
  fn get_case<T: Into<Value>, D: Into<Value>>(mp: T, ec: D) -> QucentInfo {
    let mut map = Mapping::new();
    map.insert("mixed-port".into(), mp.into());
    map.insert("external-controller".into(), ec.into());

    IQucentTemp(IQucentTemp::guard(map)).get_client_info()
  }

  fn get_result<S: Into<String>>(port: u16, server: S) -> QucentInfo {
    QucentInfo {
      port,
      server: server.into(),
      secret: None,
  ...
  assert_eq!(
    IQucentTemp(IQucentTemp::guard(Mapping::new())).get_client_info(),
...
pub struct IQucent {
  ...
  pub secret: Option<String>,
  pub dns: Option<IQucentDNS>,
  pub tun: Option<IQucentTUN>,
  pub interface_name: Option<String>,
}

#[derive(Default, Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "kebab-case")]
pub struct IQucentTUN {
...
pub struct IQucentDNS {
  ...
  pub fallback_filter: Option<IQucentFallbackFilter>,
  pub nameserver_policy: Option<Vec<String>>,
}

#[derive(Default, Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "kebab-case")]
pub struct IQucentFallbackFilter {
